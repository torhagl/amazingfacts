...
update msg model =
    case msg of
        Presses x ->
            if x == 32 then
                ( { model | ref = getRef model, fact = getFact model, press = getRandomIntInDBRange model }, Task.perform OnTime Time.now )
            else
                ( model, Cmd.none )
...
viewFact model =
    div [ class "col-12" ] [ div [ class "row" ] [ h1 [] [ text model.fact, getReference model ] ] ]
...
getRandomFromDB model =
    List.head (List.reverse (List.take (getRandomIntInDBRange model) model.db))


getRandomIntInDBRange : Model -> Int
getRandomIntInDBRange model =
    Tuple.first (Random.step (Random.int 1 ((List.length model.db) + 1)) (Random.initialSeed model.time))
